...
-- Pas obligatoire
-- --------------------------------------------------------
CREATE DATABASE `bdd_projet_annonce` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
use `bdd_projet_annonce`;

CREATE USER 'u_annonce'@'localhost' IDENTIFIED BY 'SJzEeqLb2HHeNYVV';
...
GRANT ALL PRIVILEGES ON `acces` . * TO 'u_annonce'@'localhost';
-- 
-- -------------------------------------------------------- 
...
(
    ...
    CONSTRAINT PK_MSG PRIMARY KEY (A_idannonce, U_mail),
    CONSTRAINT FK_MSG_ANNONCE FOREIGN KEY (A_idannonce) REFERENCES T_annonce (A_idannonce),
    CONSTRAINT FK_MSG_UTI FOREIGN KEY (U_mail) REFERENCES T_annonce (T_utilisateur),
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;
