...
<div class="flex-container w75 item-center">
    <div class="annonce-container formulaire w90 item-center">
        <form method="post" action="/annonce/toutesAnnonces/{$bSelect|default:''}/{$nbAnnonces|default:''}" >
            ...
            <hr>
            <div class="txtcenter">{$var.totAnnonceTrouvees} / {$var.totAnnonce} annonce{($var.totAnnonce>1)?'s':''}  au total {$annAff = ($nbAnnonces<$var.totAnnonceTrouvees)?$nbAnnonces:$var.totAnnonceTrouvees}{if $annAff>0}<span style="font-size:1.05rem">( {$annAff} annonce{($annAff>1)?'s':''} affichée{($annAff>1)?'s':''} sur la page)</span>{/if}</div>
